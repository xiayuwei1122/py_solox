...
class CPU(object):
    def __init__(self, pkgName, deviceId, platform=Platform.Android, pid=None, address=None, rsdPort=None):
    ...
    def getCoreCpuRate(self, cores=0, noLog=False, rsd=None):
        try:
            ...
            for i in range(len(coreCpuTotalTime_List1)):
                coreCpuRate = round(float((processCpuTime_2 - processCpuTime_1) / (
                            coreCpuTotalTime_List2[i] - coreCpuTotalTime_List1[i]) * 100), 2)
                coreCpuRate /= cores
    ...
    def getiOSCpuRate(self, noLog=False, lock=None, pid=None):
        """get the iOS cpu rate of a process, unit:%"""
        apm = iosPerformance(self.pkgName, self.deviceId, self._address, self._rsdPort)
        appCpuRate = round(float(apm.getPerformance(apm.cpu, lock, pid)[0]), 2)
        sysCpuRate = round(float(apm.getPerformance(apm.cpu, lock, pid)[1]), 2)
        if noLog is False:
            apm_time = datetime.datetime.now().strftime('%H:%M:%S.%f')
            f.add_log(os.path.join(f.report_dir, 'cpu_app.log'), apm_time, appCpuRate)
            f.add_log(os.path.join(f.report_dir, 'cpu_sys.log'), apm_time, sysCpuRate)
    ...
    def getCpuRate(self, noLog=False, lock=None, pid=None):
        ...
        appCpuRate, systemCpuRate = self.getAndroidCpuRate(
            noLog) if self.platform == Platform.Android else self.getiOSCpuRate(noLog, lock, pid)
...
class Memory(object):
    ...
    def getProcessMemory(self, noLog=False, lock=None, pid=None):
        """Get the app memory"""
        totalPass, swapPass = self.getAndroidMemory() if self.platform == Platform.Android else self.getiOSMemory(lock, pid)
        if noLog is False:
            apm_time = datetime.datetime.now().strftime('%H:%M:%S.%f')
...
class Network(object):
    ...
    def getNetWorkData(self, wifi=True, noLog=False, lock=None):
        """Get the upflow and downflow data, unit:KB"""
        sendNum, recNum = self.getAndroidNet(wifi) if self.platform == Platform.Android else self.getiOSNet(lock)
        if noLog is False:
            apm_time = datetime.datetime.now().strftime('%H:%M:%S.%f')
...
class FPS(object):
    ...
    def __init__(self, pkgName, deviceId, platform=Platform.Android, surfaceview=True, address=None, rsdPort=None):
        self.pkgName = pkgName
        self.deviceId = deviceId
        self.platform = platform
    ...
    def getiOSFps(self, noLog=False, lock=None):
        """get iOS Fps"""
        apm = iosPerformance(self.pkgName, self.deviceId, address=self._address, rsdPort=self._rsdPort)
        fps = int(apm.getPerformance(apm.fps, lock))
        if noLog is False:
            apm_time = datetime.datetime.now().strftime('%H:%M:%S.%f')
            f.add_log(os.path.join(f.report_dir, 'fps.log'), apm_time, fps)
        return fps, 0

    def getFPS(self, noLog=False, lock=None):
        """get fps、jank"""
        fps, jank = self.getAndroidFps(noLog) if self.platform == Platform.Android else self.getiOSFps(noLog, lock)
        return fps, jank


class GPU(object):
    def __init__(self, pkgName, deviceId, platform=Platform.Android, address=None, rsdPort=None):
        self.pkgName = pkgName
        self.deviceId = deviceId
        self.platform = platform
        self._address = address
    ...
    def getiOSGpuRate(self, lock=None):
        apm = iosPerformance(self.pkgName, self.deviceId, address=self._address, rsdPort=self._rsdPort)
        gpu = apm.getPerformance(apm.gpu, lock=lock)
        return gpu

    def getGPU(self, noLog=False, lock=None):
        gpu = self.getAndroidGpuRate() if self.platform == Platform.Android else self.getiOSGpuRate(lock)
        if noLog is False:
            apm_time = datetime.datetime.now().strftime('%H:%M:%S.%f')
...
class Disk(object):
    def __init__(self, deviceId, platform=Platform.Android, address=None, rsdPort=None):
        self.deviceId = deviceId
        self.platform = platform
        self._address = address
        self._rsdPort = rsdPort
...
class iosPerformance(object):
    ...
    def __init__(self, pkgName, deviceId, address=None, rsdPort=None):
        self.pkgName = pkgName
        self.deviceId = deviceId
        self.apm_time = datetime.datetime.now().strftime('%H:%M:%S.%f')
    ...
    def getPerformance(self, perfType: DataType, lock=None, pid=None):
        if perfType == DataType.NETWORK:
            ...
            perf = iosP.Performance(tidevice.Device(udid=self.deviceId), [perfType], self._address, self._rsdPort)
            perf_value = perf.start(self.pkgName, callback=self.callback, lock=lock, pid=pid)
        return perf_value
...
class AppPerformanceMonitor(initPerformanceService):
    ...
    def __init__(self, pkgName=None, platform=Platform.Android, deviceId=None,
                 ...
                 duration=0, address=None, rsdPort=None):
        self.pkgName = pkgName
        self.deviceId = deviceId
        self.platform = platform
        self.surfaceview = surfaceview
    ...
    def collectCoreCpu(self):
        _cpucore = CPU(self.pkgName, self.deviceId, self.platform, pid=self.pid, address=self.address,
    ...
    def collectDisk(self):
        _disk = Disk(self.deviceId, self.platform, address=self.address, rsdPort=self.rsdPort)
