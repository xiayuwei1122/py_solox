...
from solox.public.ios_device.remote.remote_lockdown import RemoteLockdownClient
from solox.public.ios_device.servers.Instrument import InstrumentServer
...
def iter_network_flow(d: BaseDevice, rp: RunningProcess, address=None, rsdPort=None, lock=None) -> Iterator[Any]:
    ...
    if rp.get_version_lists() == 1:
        ...
        with lock:
            ...
            nstat = networking(rpc, result_queue)
            rsd_1.close()
            rpc.stop()
        # print(data)
        # for nstat in data:
...
class Performance():
    ...
    def start(self, bundle_id: str, callback: CallbackType = None, lock=None):
        if not callback:
        ...
        self._thread_start(callback, lock=lock)

    def _thread_start(self, callback: CallbackType, lock=None):
        print(self._perfs)
        iters = []
